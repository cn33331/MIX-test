...
class SimpleRPCServer:
    ...
    参数:
        simulate_registration_failure: 是否模拟注册失败（用于测试）
        registration_failure_rate: 注册失败概率（0-1，0表示不失败，1表示总是失败）
    """
    def __init__(self, host="0.0.0.0", port=7801, simulate_registration_failure=False, registration_failure_rate=0.5):
        global services
        ...
        # 客户端注册字典：{client_id: client_identity}
        self.registered_clients = {}
        
        # 测试模式：模拟注册失败
        self.simulate_registration_failure = simulate_registration_failure
        self.registration_failure_rate = registration_failure_rate
    ...
    def _process_request(self, request_data, client_id, remote_id_from_header=None):
        """
        处理请求（支持多种协议格式）
        ...
        try:
            ...
            if is_jsonrpc2:
                ...
                # MIX_2.0 格式
                request_id = data.get("id")
                # 优先使用 header 中的 remote_id
                remote_id = remote_id_from_header if remote_id_from_header else data.get("remote_id")
                ...
                if "." in method_full:
                    ...
                    remote_id = remote_id_from_header if remote_id_from_header else data.get("remote_id", method_full)
                    method = data.get("method", "")
                args = data.get("args", [])
                kwargs = data.get("kwargs", {})
            ...
            # 处理 __MIX_CLIENT_MANAGER__ 服务（不需要注册验证）
            if remote_id == "__MIX_CLIENT_MANAGER__":
                if method == "hello":
                    # 客户端注册
                    client_identity = args[0] if args else "unknown"
                    
                    # 测试模式：模拟注册失败
                    if self.simulate_registration_failure:
                        import random
                        if random.random() < self.registration_failure_rate:
                            print(f"[测试模式] 模拟注册失败: {client_identity} (ID: {client_id.hex()})")
                            return {
                                "version": "MIX_2.0",
                                "request_id": request_id,
                                "error": {
                                    "code": -32603,
                                    "message": "Registration failed (simulated for testing)"
                                }
                            }
                    
                    self.registered_clients[client_id] = client_identity
                    print(f"客户端注册成功: {client_identity} (ID: {client_id.hex()})")
                    return {
                        "version": "MIX_2.0",
                        "request_id": request_id,
                        "result": {"status": "success", "session_id": "test_session"}
                    }
                elif method == "bye":
                    # 客户端断开
                    if client_id in self.registered_clients:
                        client_identity = self.registered_clients.pop(client_id)
                        print(f"客户端断开: {client_identity} (ID: {client_id.hex()})")
                    return {"version": "MIX_2.0", "request_id": request_id, "result": {"status": "success"}}
                else:
                    return self._error_response(request_id, f"Unknown method: {method}", is_jsonrpc2)
            
            # __server__ 服务不需要注册验证
            if remote_id == "__server__":
            ...
            if client_id not in self.registered_clients:
                error_msg = f"unregistered client {client_id}. This may be a spurious message from a previous session"
    ...
    def _run(self):
        ...
        while self.running:
            try:
                # RPC8格式（三个参数）: [client_id, remote_id, request_data]
                # RPC7格式（两个参数，兼容）: [client_id, request_data]
                ...
                if len(msg_parts) >= 3:
                    ...
                    remote_id = msg_parts[1].decode('utf8')
                    request_data = msg_parts[2].decode('utf8')
                    
                    print(f"收到请求 from {client_id.hex()} to {remote_id}: {request_data}")
                    
                    # 处理请求
                    response = self._process_request(request_data, client_id, remote_id)
...
def main():
    ...
    命令行参数:
        --test-reg-fail: 启用注册失败模拟模式
        --fail-rate <rate>: 设置注册失败概率（0-1，默认0.5）
    """
    import sys
    
    # 解析命令行参数
    simulate_reg_fail = "--test-reg-fail" in sys.argv
    fail_rate = 0.5
    
    # 查找 --fail-rate 参数
    for i, arg in enumerate(sys.argv):
        if arg == "--fail-rate" and i + 1 < len(sys.argv):
            try:
                fail_rate = float(sys.argv[i + 1])
            except ValueError:
                print("警告: --fail-rate 参数值无效，使用默认值 0.5")
    
    # 创建服务器
    server = SimpleRPCServer(
        "0.0.0.0", 
        7801,
        simulate_registration_failure=simulate_reg_fail,
        registration_failure_rate=fail_rate
    )
    
    # 打印启动信息
    if simulate_reg_fail:
        print(f"\n=== 测试模式已启用 ===")
        print(f"注册失败概率: {fail_rate * 100}%")
        print("=====================\n")
