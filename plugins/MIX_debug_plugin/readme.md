...
- **批量操作**：支持批量连接和断开选中通道（工具栏提供“连接/断开”按钮）
- **状态可视化**：状态列使用颜色区分（绿色=已连接、灰色=未连接、黄色=连接中、红色=错误）
- **配置导入导出**：自动保存和加载通道配置
...
- **实时日志**：显示所有操作和设备响应
- **通道隔离日志**：每个通道拥有独立日志页签，总日志聚合所有通道输出
- **历史记录**：保存已发送的命令历史
- **右键菜单**：支持清空、添加、删除等操作
...
├── MIX_debug_plugin.ui      # Qt Designer 界面文件（工作台式三段布局）
...
采用 **三段可拖拽工作台布局**，所有区域均通过 `QSplitter` 连接，用户可自由调节宽度：

```
┌────────────────┬─────────────────────────────┬─────────────────┐
│  通道管理       │      命令栏 + 命令详情        │ 历史指令 │ 指令序列 │
│  + 配置 连接 断开│  [命令] [参数...] [超时] [发送]│                 │
│  表格（状态/操作）│  ─────────────────────────  │  列表 / 操作区   │
│                │      总日志 │ Slot1 │ ...     │                 │
│                │  ─────────────────────────  │                 │
│                │      通道隔离日志（深色终端风格）              │
...
- **左栏**：通道管理面板（固定宽度，不随窗口拉伸），顶部工具栏提供新增、配置、批量连接/断开。
- **中栏**：核心工作区。上方为单行命令栏；中部为命令详情/日志的垂直分割，日志区采用深色终端风格。
- **右栏**：历史指令与指令序列以 Tab 形式组织（固定宽度）。

## 样式

界面样式通过 `.ui` 文件中的统一 QSS 管理，主色调为 `#0078d4`：

- 主要操作按钮（发送）使用实心主色。
- 次要操作按钮（执行序列）使用浅色主色描边。
- 输入框聚焦时边框变为主色，日志区使用等宽字体。
- 标签选中态使用主色下划线。
...
- v4: 优化协议实现，添加详细调试日志；重构为三段可拖拽工作台布局，统一 QSS 主色，通道状态颜色化
